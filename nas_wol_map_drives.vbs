...
'Requires WOL.EXE from Gammadyne
'http://www.gammadyne.com/cmdline.htm#wol

'This script will turn on the NAS via Wake On Lan, ping it to see if it is awake and then map the drives.
'Go through the script and change hostname/ipaddress, user name, password etc where specified.

'Turn on the NAS via Wake On Lan
Sub wol( cmd )
    Dim wshShell
    Set wshShell = CreateObject( "WScript.Shell" )
	'Change the path so the script can locate wol.exe
    wshShell.Run "c:\path_to_wol_install\wol.exe " & ( cmd ), 0, False
